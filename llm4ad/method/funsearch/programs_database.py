...
class ProgramsDatabase:
    ...
    def reset_islands(self) -> None:
        ...
        # Lower scores are better. We sort best scores after adding minor
        # noise to break ties, then reset the weaker half.
        scores = np.asarray(self._best_score_per_island, dtype=float)
        indices_sorted_by_score: np.ndarray = np.argsort(
            scores + np.random.randn(len(scores)) * 1e-6)
        num_islands_to_reset = self._config.num_islands // 2
        keep_islands_ids = indices_sorted_by_score[:len(indices_sorted_by_score) - num_islands_to_reset]
        reset_islands_ids = indices_sorted_by_score[len(indices_sorted_by_score) - num_islands_to_reset:]
...
class Island:
    ...
    def get_prompt(self) -> tuple[str, int]:
        ...
        # Scores are minimization objectives, so lower scores are sampled more often.
